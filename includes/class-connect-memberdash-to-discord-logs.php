 ...
 */
class MemberDash_Discord_Add_On_Logs {
	function __construct() {
		// Clear all existing logs.
		add_action( 'wp_ajax_ets_memberdash_discord_clear_logs', array( $this, 'ets_memberdash_discord_clear_logs' ) );
	}
	 ...
	 */
	public static $log_file_name = 'memberdash_discord_api_logs.txt';
	 ...
	 * @return None
	 */
	public function ets_memberdash_discord_clear_logs() {
		...
		if ( ! wp_verify_nonce( $_POST['ets_memberdash_discord_nonce'], 'ets-memberdash-discord-ajax-nonce' ) ) {
			wp_send_json_error( 'You do not have sufficient rights', 403 );
			exit();
		}
		try {
	...
	static function write_api_response_logs( $response_arr, $user_id, $backtrace_arr = array() ) {
		...
		$log_api_response = sanitize_text_field( trim( get_option( 'ets_memberdash_discord_log_api_response' ) ) );
		$uuid             = sanitize_text_field( trim( get_option( 'ets_memberdash_discord_uuid_file_name' ) ) );
...
new MemberDash_Discord_Add_On_Logs();
